...
class GroupFoodTable extends React.Component<Props, State> {
  ...
  componentDidMount() {
    this.setState({
      rows: this.props.foodPurchases.map((x) => this.createData(x)),
    });
  }
  ...
  onDelete = async (id: any) => {
    ...
    console.log("remove food: ", this.state.rows);
    this.props.onChange(this.state.rows);
  };
  ...
  render() {
    ...
    const tableFoodHead = ["Datum", "Gewicht (kg)"];
        ...
        <Table className={classes.table}>
          <TableHead className={classes[tableHeaderColor + "TableHeader"]}>
            ...
            {checkRole(
              ...
              {tableFoodHead.map((prop: any, key: any) => {
                return (
                  <TableCell
            ...
            {this.state.rows
              ...
              .map((row) => {
                ...
                this.props.userDetail.roles != undefined
                  ? this.props.userDetail.roles
                  : [],
                ["Manager", "Voedingsbeheerder"]) ? <TableCell className={classes.selectTableCell}>
                {row.isEditMode ? (
                  <>
                    <IconButton
                      aria-label="done"
...
export default withHooksComponent(withStyles(tableStyle)(GroupFoodTable));
