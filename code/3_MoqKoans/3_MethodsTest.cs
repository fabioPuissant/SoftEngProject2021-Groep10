...
namespace MoqKoansCore
{
	...
	public class Moq3_MethodsTest : Koan
	{
        ...
        delegate void ArgumentGreaterThanZeroDelegate(int input);
		...
		public void UsingWhatYouHaveLearned_CreateACompletelyFunctionalIVolumeMock()
		{
            ...
            ArgumentGreaterThanZeroDelegate greaterThanZeroCheck = x =>
            {
                if (x < 0)
                {
            ...
            mock.Setup(x => x.Louder(It.IsAny<int>())).Returns<int>(input =>
            {
                greaterThanZeroCheck(input);
            ...
            mock.Setup(x => x.Quieter(It.IsAny<int>()))
                .Returns<int>(input =>
                {
                    ...
                    currentVolume = currentVolume- input > -1 ? (currentVolume - input) : 0;

                    return currentVolume;
                });

            mock.Setup(x => x.CurrentVolume()).Returns(() => currentVolume.ToString());
