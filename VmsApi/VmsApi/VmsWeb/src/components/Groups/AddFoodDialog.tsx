...
class AddFoodDialog extends React.Component<Props, State> {
  refs: any;
  ...
  handleSave = async () => {
    ...
    await doJsonPostApiCall(
      ...
      { GroupNumber: this.state.groupNumber, date: date.toISOString(), amount: this.state.weight},
      (weightData: any) => {
        this.setState({
          groupNumber: 0,
          date: "",
  ...
  render() {
    ...
    return (
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Voer toevoegen"}</DialogTitle>
        <DialogContent>
...
export default withHooksComponent(withStyles(tableStyle)(AddFoodDialog));
